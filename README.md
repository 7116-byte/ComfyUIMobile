...
> 当前版本：v0.1.39。首要兼容 ComfyUI 0.28.0、ComfyUI Frontend 1.45.21。
...
复杂画板、遮罩、曲线等控件会保留原值并标记为特殊控件，可进入“高级编辑”使用服务器原始前端。高级编辑按服务器上的真实工作流路径打开，并带入手机端尚未保存的参数；它会优先切换为简体中文并显示原有连线，返回时重新同步原生参数页。
