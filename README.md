...
> 当前版本：v0.1.3。首要兼容 ComfyUI 0.28.0、ComfyUI Frontend 1.45.21。
...
- 可直接选择带 ComfyUI 工作流信息的 PNG、WebP、AVIF 图片导入，解析逻辑与服务器网页前端一致
- 隐藏 WebView 调用服务器自己的 ComfyUI 前端，使用真实节点定义执行 `loadGraphData()` 和 `graphToPrompt()`
- 参数页按真实节点组成可展开卡片，一个节点内显示它在网页中的可编辑控件
- 自动排除“绕过”和“永不执行”的节点；支持 rgthree 整组开关，切换后立即刷新当前启用流程
- 原生参数控件：提示词、文本、数字、滑杆、开关、下拉、图片和视频上传；常用参数自动显示中文名称
