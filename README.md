...
> 当前版本：v0.1.4。首要兼容 ComfyUI 0.28.0、ComfyUI Frontend 1.45.21。
...
- 选择工作流后自动进入参数页；参数按真实执行链顺序组成可展开部件卡片
- 只保留连接到有效输出的执行链，同时排除“绕过”“永不执行”和未连线分支；支持 rgthree 整组开关
...
- 可长按输出部件加入本地缓存白名单；仅缓存本 App 提交且命中白名单的输出，不缓存浏览器任务
